...
describe("Verify Other To Someone Else Functionality", () => {
  ...
  it("Select Someone Else Tab and validate expected fields appear", () => {
    cy.get(giftCardPage.sendToOtherTab).click();

    cy.get(giftCardPage.selectedOtherTab).should("exist");

    cy.get(giftCardPage.FirstNameLastNameHeaders)
      .eq(3)
      .should(($div) => {
        expect($div.text().trim()).equal("Recipient Email");
      });

    cy.get(giftCardPage.recipientEmailPlaceholder).should("exist");

    cy.get(giftCardPage.FirstNameLastNameHeaders)
      .eq(4)
      .should(($div) => {
        expect($div.text().trim()).equal("Message for Recipient");
      });

    cy.get(giftCardPage.messageForRecipientPlaceholder).should("exist");
  });
  ...
  it("Select Other and Validate page updates to reflect this", () => {
    cy.get(giftCardPage.radioOther).click();

    cy.get(giftCardPage.otherInputField).should("exist");

    cy.get(giftCardPage.otherFieldError).should("have.text", "Required");

    cy.get(giftCardPage.totalPrice).eq(0).should("have.text", "$0.00");
  ...
  it("Type invalid input into the Other field and validate the corresponding error message appears", () => {
    cy.get(giftCardPage.otherInputField).type(vars.otherFieldInvalidText);
  ...
  it("Type below minimum input into the Other field and validate the corresponding error message appears", () => {
    cy.get(giftCardPage.otherInputField).type(vars.otherFieldBelowMinGiftCard);

    cy.get(giftCardPage.otherFieldError).should(
      "have.text",
      "The minimum spend is $25 and the maximum spend is $1000."
    );
    ...
    cy.get(giftCardPage.otherInputField).clear();
  });

  it("Type above maximum input into the Other field and validate the corresponding error message appears", () => {
    cy.get(giftCardPage.otherInputField).type(vars.otherFieldAboveMaxGiftCard);
  ...
  it("Click other radio again and enter valid amount, confirm there are no error messages and page updates correctly", () => {
    cy.get(giftCardPage.radioOther).click();

    cy.get(giftCardPage.otherInputField).type(vars.otherFieldValidGiftCard);

    cy.get(giftCardPage.otherFieldError).should("not.have.text", "Required");

    cy.get(giftCardPage.totalCostValue)
      .eq(0)
      .should("contain", vars.otherFieldValidAmount);
  });

  it("Check error messages for Recipient Email Address", () => {
    cy.get(giftCardPage.recipientEmailField).type("t");

    cy.get(giftCardPage.recipientEmailFieldError).should(
      "have.text",
      "Please enter a valid email"
    );

    cy.get(giftCardPage.recipientEmailField).clear();
  ...
  it("Enter Recipient email and populate message field with text", () => {
    cy.get(giftCardPage.recipientEmailField).type(vars.emailAddress2);

    //validate errors do not appear for email
    cy.get(giftCardPage.recipientEmailField).should("not.have.text");

    cy.get(giftCardPage.messageForRecipientField).type(vars.testMessage);
  });
  ...
  it("Validate Content of Summary page", () => {
    ...
    cy.get(confirmationPage.giftCardValue).should(($div) => {
      expect($div.text().trim()).contains(vars.otherFieldValidGiftCard);
    });
    ...
    cy.get(confirmationPage.paymentAmount).should(($div) => {
      expect($div.text().trim()).contains(vars.otherFieldValidAmount);
    });
  ...
  it(
    ...
    () => {
      ...
      cy.get(successPage.paymentSuccessMessage)
        .children("p")
        .eq(5)
        .should("contain", vars.otherFieldValidGiftCard);
