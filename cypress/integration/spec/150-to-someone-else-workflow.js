...
describe("Verify $150 To Me Functionality", () => {
  ...
  it("Select $150 and Validate page updates to reflect this", () => {
    cy.get(giftCardPage.radio150).click();

    cy.get(giftCardPage.giftCardAmountAfterSelection)
      .invoke("attr", "data-amount-payment-amount")
      .should("eq", "120");

    cy.get(giftCardPage.totalPrice).eq(0).should("have.text", "$120.00");
  ...
  it("Validate Content of Summary page", () => {
    ...
    cy.get(confirmationPage.giftCardValue).should(($div) => {
      expect($div.text().trim()).equal("$150.00");
    });
    ...
    cy.get(confirmationPage.paymentAmount).should(($div) => {
      expect($div.text().trim()).equal("$120.00");
    });
