...
describe("Verify $50 To Someone Else Functionality", () => {
  ...
  it("Select $50 and Validate page updates to reflect this", () => {
    ...
    cy.get(giftCardPage.totalPrice).eq(0).should("have.text", "$40.00");
  ...
  it("Check error message for Recipient field", () => {
    ...
    //expected error is speculative as no reference in code to actual error, nor documentation
    cy.getCookie(giftCardPage.messageForRecipientError).should(
      "have.text",
      "Max Character Limit Exceeded"
    );

    cy.get(giftCardPage.messageForRecipientField).clear();
  });
  ...
  it("Validate Content of Summary page", () => {
    ...
    cy.get(confirmationPage.giftCardValue).should(($div) => {
      expect($div.text().trim()).equal("$50.00");
    });
    ...
    cy.get(confirmationPage.paymentAmount).should(($div) => {
      expect($div.text().trim()).equal("$40.00");
    });
